...
export const SkillsGrid = ({ skills }: SkillsGridProps) => {
  return (
    <ul
      class="mx-auto grid w-full cursor-default grid-cols-2 items-center justify-center gap-8 px-2 md:grid-cols-6 md:px-0">
      {getSkills(skills).map(({ Icon, name }, i) => (
        <li
          ...
          <Icon />
          <span
            class="text-center text-sm text-gray-700 transition-all duration-700 group-hover:text-base group-hover:text-gray-900 dark:text-gray-400 group-hover:dark:text-gray-100">
            {name}
          </span>
        </li>
      ))}
